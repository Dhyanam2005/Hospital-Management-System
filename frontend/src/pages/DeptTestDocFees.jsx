...
function DeptTestDocFees() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await authFetch(`${API_BASE_URL}/deptTestDocFees?startDate=${s}&endDate=${e}`);
  ...
  const columns = [
    { field: 'Test Date',       headerName: 'Test Date',   flex: 1, minWidth: 120 },
    { field: 'Test Department', headerName: 'Department',  flex: 1, minWidth: 150 },
    { field: 'Test Name',       headerName: 'Test Name',   flex: 1, minWidth: 160 },
    { field: 'Doctor',          headerName: 'Doctor',      flex: 2, minWidth: 180 },
    {
      field: 'Total Fee Collected', headerName: 'Total Fee', flex: 1, minWidth: 130,
      renderCell: p => <span style={{ fontWeight: 700, color: '#059669' }}>₹ {p.value}</span>,
    },
  ];
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
        ...
        <Box>
          <Typography variant="h5" fontWeight={700} color="text.primary">Department Test Doctor Fees</Typography>
          <Typography variant="caption" color="text.secondary">Fee collected per department, test, and doctor for a date range</Typography>
        </Box>
      </Box>
