...
function DoctorWiseRegistration() {
  ...
  const fetchData = async () => {
    if (!startDate || !endDate) {
      setAlert('Please select both start date and end date.');
      return;
    }
    setLoading(true);
    setAlert('');
    try {
      ...
      const res = await authFetch(`${API_BASE_URL}/doctorWiseRegistrationFees?startDate=${s}&endDate=${e}`);
  ...
  const columns = [
    ...
    {
      field: 'No of Patients', headerName: 'No. of Patients', flex: 1, minWidth: 140,
      renderCell: p => (
        <Chip label={p.value} size="small"
          sx={{ fontSize: 11, fontWeight: 700, height: 22, backgroundColor: '#eff6ff', color: '#2563eb' }} />
      ),
    },
    {
      field: 'Total Fee Collected', headerName: 'Total Fee Collected', flex: 1, minWidth: 160,
      renderCell: p => (
        <span style={{ fontWeight: 700, color: '#059669' }}>₹ {p.value}</span>
      ),
    },
  ];
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
        ...
        <Box>
          <Typography variant="h5" fontWeight={700} color="text.primary">Doctor-wise Registration Fees</Typography>
          <Typography variant="caption" color="text.secondary">View registration fee summary grouped by doctor for a date range</Typography>
        </Box>
      </Box>
      ...
      {data.length > 0 && (
        <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 2, overflow: 'hidden' }}>
          <Box sx={{
            ...
            <Button
              size="small" variant="outlined" onClick={() => exportToExcel(data)}
